...
- **확인 카드**: 유료 생성 전에 만들 것·레퍼런스·방향·문구·사용 도구·비용(실제 크레딧과 잔액)을 한 장으로 보여줍니다. "이대로 1장 만들기" 또는 "2장 만들어 더 나은 것 고르기"를 골라야 생성이 시작됩니다. 영상은 기준 이미지와 영상, 두 번 확인합니다.
- **결과 뒤에 할 수 있는 일**: 제품이 원본과 같은지 ✓ 표시로 확인하고, 광고용 고화질(2K·4K)로 키우기, 다른 비율로 늘리기, 이 이미지로 영상 만들기를 이어서 요청할 수 있습니다.
- **여러 방향을 비교하고 싶다면** "여러 안 보여줘", "콘셉트 3안", "레퍼런스 먼저 보여줘"라고 말하세요. 자세히 보기 방식으로 바뀝니다.
...
Chany's Studio 2.10.0은 두 축, 하나의 전문 크리에이티브 디렉션 계층과 하나의 공통 미디어 실행 루프를 함께 사용합니다.
