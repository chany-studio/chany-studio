...
- **확인 카드**: 유료 생성 전에 만들 것·레퍼런스·방향·문구·사용 도구·비용(1장·2장 실제 크레딧과 잔액)을 한 장으로 보여줍니다. 레퍼런스 사진을 참고 입력으로 보낼 때는 "다른 사람의 작품을 보낸다"는 줄이 따로 나오며, 승인해야만 보냅니다. "이대로 1장 만들기" 또는 "2장 만들어 더 나은 것 고르기"를 골라야 생성이 시작됩니다. 영상은 기준 이미지와 영상, 두 번 확인합니다.
...
- **전문 용어를 몰라도 됩니다**: 평소 말로 요청하면 전문 촬영 용어로 바꿔 만들고 "이렇게 이해했어요"로 알려줍니다. "느낌 직접 고를래", "이건 뭐라고 해?"라고 물어도 됩니다. 아래 [전문 용어 몰라도 괜찮아요](#전문-용어-몰라도-괜찮아요)를 보세요.
...
사진·영상 용어를 몰라도 평소 말로 요청하면 됩니다. 플러그인이 전문 용어로 바꿔 만들고, 어떻게 이해했는지 한 줄로 보여줍니다.

| 이렇게 말하면 | 전문가는 이렇게 부릅니다 |
| --- | --- |
| 배경 흐리게, 제품만 또렷하게 | 아웃포커싱(얕은 심도) |
| 화장품 화보처럼 반짝이게 | 뷰티디쉬 조명 |
| 따뜻한 창가 햇살 | 골든아워 자연광 |
| 위에서 내려다본 | 탑뷰(플랫레이) |
| 글자 넣을 자리 남겨서 | 네거티브 스페이스(카피 여백) |
| 천천히 다가가는 영상 | 푸시인 |
| 제품이 빙글 도는 영상 | 턴테이블 |
...
"느낌 직접 고를래"라고 하면 빛·각도·배경(영상이면 카메라 움직임)을 쉬운 말 선택지로 묻는 질문 카드가 나옵니다. Claude Cowork에서는 질문 카드로, ChatGPT Work에서는 선택 입력이나 번호 선택으로 나옵니다. 답하고 나면 다음에도 그대로 쓸 수 있는 전문가식 요청문을 알려줍니다.
...
Chany's Studio 2.11.0은 두 축, 하나의 전문 크리에이티브 디렉션 계층과 하나의 공통 미디어 실행 루프를 함께 사용합니다.
...
Moai와 함께 쓰는 방법(개발자용)은 [고급·개발자 안내](ADVANCED.md#moai와-함께-쓸-때)로 옮겼습니다.
...
실행 전에는 현재 호스트가 정확한 모델 선택을 노출하는지 확인하고 `요청 기본값: gpt_image_2_5`와 `실제 해석 모델`을 함께 표시합니다. 확인할 수 없는 공급자 기본값을 GPT Image 2.5라고 부르지 않습니다. 다른 모델은 사용자가 지정했거나, 승인된 프로젝트 지침에 기록됐거나, 현재 기능 확인에서 `gpt_image_2_5`가 필수 작업을 처리할 수 없음이 드러난 경우에만 해당 산출물 범위를 적고 승인받아 사용합니다. 단순 품질 불만, 타임아웃 또는 실패 응답만으로 자동 전환하지 않으며, 모델이나 공급자가 달라지면 견적·유료 승인·크리에이티브 승인을 새로 받습니다.
...
최신 신호는 Pinterest Predicts와 Pinterest Palette를 후보 선별 뒤에만 사용합니다. 검색어에는 연도·트렌드명·색·무드를 넣지 않습니다. 한 콘셉트에는 원칙적으로 중심 신호 하나와 선택적 보조 포인트 하나만 쓰며, 브랜드·메시지·제품 진실·접근성에 맞지 않으면 유행을 쓰지 않습니다. 플랫폼 규격, 상세페이지 구조와 GPT Image 2.5 프롬프팅은 실행 시점의 공식 문서로 확인하지만 그 문서의 예시 이미지는 레퍼런스로 취급하지 않습니다.
