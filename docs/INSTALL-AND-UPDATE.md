...
명시 호출은 ChatGPT Work에서 `@스킬명`, Codex에서 `$스킬명`을 사용합니다. Claude Cowork에서는 `/project-studio` 슬래시 명령을 제공하며, 나머지 전문 스킬은 자연어 요청에 맞춰 자동 선택합니다.
...
3. 버전 `2.1.1`과 `chany-project`를 포함한 전문 스킬 목록을 확인합니다.
