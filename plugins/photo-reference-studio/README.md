...
목적·보는 사람·게시 위치를 먼저 파악하고 빠진 핵심만 1~3개 질문합니다. 첨부 레퍼런스와 승인된 기획은 다시 고르게 하지 않습니다. 새 생성·AI 부분 수정·일반 편집 중 적합한 방식을 추천하며, 전문용어나 모델 이름을 알 필요는 없습니다. 프로젝트 설정에도 같은 최소 질문을 적용하고 별도의 긴 선호도 설문은 추가하지 않습니다.

이미지 부분 편집은 `chany-image-edit`, 기존 영상의 AI 수정은 `chany-campaign-video`의 편집 경로, 컷·자막·오디오·구간 조립은 `chany-video-assembly`가 맡습니다. 실제 도구 연결을 확인하고 원본 보존 여부를 검수합니다. [쉬운 요청 예시](../../docs/USER-GUIDE.md#목적을-말하면-제작-방식까지-추천해요).
...
생성형 정적 이미지와 부분 편집은 **GPT Image 2.5**(`gpt_image_2_5`)가 기본입니다. 호출 전 실제 선택 가능 여부를 확인하고, 확인되지 않은 공급자 기본값을 GPT Image 2.5라고 부르지 않습니다. 사용자 지정·승인된 프로젝트 정책·필수 기능 부재 또는 구체적인 작업 적합성에 따라 대체 모델을 제안할 수 있습니다. 이유·적용 범위·입력·비용을 기존 확인 카드에서 승인받은 뒤 사용하며, 불확실하면 기본값을 유지합니다. 변경된 모델·공급자는 해당 견적과 승인을 새로 받아야 합니다. 영상·음성·일반 편집·클립 조립에는 이 기본값을 강제하지 않습니다.

유료 생성 전에는 연결된 도구의 **현재 스키마와 라이브 카탈로그**를 확인합니다. 무료 견적이 있으면 확인하고, 목적·방향·문구·입력 전송·실제 모델과 비용을 한 카드에 보여줍니다. 정확한 프롬프트·옵션·수량·한도는 내부 기록에 묶고 전문 프롬프트는 요청할 때만 공개합니다. 기억 속 모델명이나 이전 가격표를 현재 사실처럼 사용하지 않습니다.
...
- 연결된 Higgsfield MCP에서는 현재 스키마가 노출하는 GPT Image 2.5 정적 이미지·영상·편집·오디오 등을 승인된 범위에서 실행합니다. 다른 모델은 위의 범위별 추천·승인 정책을 따릅니다.
