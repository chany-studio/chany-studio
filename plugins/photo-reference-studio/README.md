...
유료 생성 전에는 연결된 도구의 **현재 스키마와 라이브 카탈로그**를 확인합니다. 무료 견적·비용 미리보기가 있으면 먼저 실행하고, 최종 프롬프트, 입력별 역할, 확인된 모델·워크플로와 옵션, 서버 조정값, 공급자가 반환한 크레딧·잔액 정보, 출력 수와 배치 한도를 보여준 뒤 승인을 받습니다. 기억한 모델명이나 이전 가격표를 현재 사실처럼 사용하지 않습니다.
