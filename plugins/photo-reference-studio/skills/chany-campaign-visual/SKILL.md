...
4. Use a selected reference only for composition, camera, light, palette, material, props, depth, and mood. Exclude its subject, person, packaging, logo, copy, price, and branded layout. For a supplied product, follow [product insertion](../chany-studio/references/product-insertion.md): separate physical identity from source-photo pose, plan a reference reshoot, prepare a clean input only if needed, use separately roled inputs when supported, and check product identity and reference composition independently. Preserve the original pose for explicitly bounded edits, not by default for a new scene. Then offer optional upscale and extra ratios when needed.
