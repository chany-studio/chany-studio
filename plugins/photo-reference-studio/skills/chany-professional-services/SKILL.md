...
Pass the complete `industry_direction` object to downstream production skills. Shared fields are the stable interface; keep professional-service-only detail under `domain_extensions`. `reference_route` must match the central taxonomy exactly. Keep `l2` an empty string and `query_count` at `1` unless one direct subtype is confidently applicable, then set the count to `2`. Do not put audience, style, location, mood, camera, lighting, color, emotion, campaign, platform, or layout words in the route.
