...
| short Instagram Reel, TikTok, or YouTube Short plus the user's product; “제품만 바꿔서 만들어줘” | `chany-viral-product-remake` | one fast chain that defaults to an original product-specific remake, or an authorized product swap when usable source media and rights are confirmed |
...
| executable, coded, or motion-rich landing experience | not produced by Chany's Studio; `chany-detail-page` supplies the static plan | static modules and copy, with a clear note that executable implementation was not performed |
...
- Public-library search is not Ads Manager reporting or operation. Requested performance data analysis belongs to `chany-marketing-brief`. Account writes, budgets and activation remain separately approved actions, not part of creative production.
...
- Static images, copy, and module plans for a landing page belong to `chany-detail-page`. Executable or motion-rich landing implementation is outside this plugin; return the completed static plan and state that executable implementation was not performed.
...
- `chany-viral-product-remake` is the beginner-facing coordinator only for one short reference-to-product video. A third-party public link defaults to an original remake of transferable mechanisms; a literal product swap requires authorized inspectable source media and explicit modification rights. The URL alone never authorizes downloading or transforming the source.
