...
test("both manifests publish the 2.10.1 conversion-creative release", async () => {
  const [claude, codex] = await Promise.all([
    readPluginFile(".claude-plugin/plugin.json").then(JSON.parse),
    readPluginFile(".codex-plugin/plugin.json").then(JSON.parse),
  ]);

  assert.equal(claude.version, "2.10.1");
  assert.match(codex.version, /^2\.10\.1(?:\+codex\.[a-z0-9.-]+)?$/i);
