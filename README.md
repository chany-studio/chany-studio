...
영상은 기준 이미지와 영상, 이렇게 확인 카드가 두 번 나옵니다. 여러 방향을 비교해 보고 싶다면 "여러 안 보여줘", "콘셉트 3안"이라고 말하세요. 자세히 보기 방식으로 바뀝니다. 레퍼런스를 직접 고르고 싶으면 "레퍼런스 먼저 보여줘", 레퍼런스 없이 바로 만들고 싶으면 "레퍼런스 없이 바로 만들어줘"라고 하면 됩니다. 레퍼런스는 사이트를 정하지 않으면 Pinterest 5장 + Meigen 5장, 총 10장을 보여줍니다.
...
업종별 구매 여정·증거·연출·컴플라이언스 판단과 공통 제작 워크플로를 결합해 **광고·홍보·마케팅·상세/랜딩·콘텐츠**를 기획하고 제작하는 ChatGPT Work/Codex 및 Claude 플러그인입니다. Chany's Studio 2.11.0은 출처별 시각 레퍼런스, 사용자가 허가한 브라우저 탭 캡처 기반 영상 레퍼런스 분석, 초보자의 말로 세 가지 전문 콘셉트를 설계하는 크리에이티브 디렉션, 최신 공식 제작 규칙과 브랜드 적합 트렌드를 반영한 GPT Image 2.5 프롬프트, 제한된 미디어 제작 루프를 하나의 흐름으로 연결합니다.
...
- **카드 ① 프로젝트**: 무엇을 파는지, 주로 만들 것(여러 개 선택), 올릴 곳(여러 개 선택), 원하는 분위기
- **카드 ② 제작 방식**: 레퍼런스 찾는 곳(Pinterest·Meigen 자동 추천 / Higgsfield 광고 템플릿 등), 기본 품질과 장수, 레퍼런스 사진을 참고로 보낼지, 기본 비율
- **카드 ③ 설정 승인**: 저장할 내용을 확인하고 `승인 후 생성 (권장)`·`설계 수정`·`취소` 중에서 고릅니다.
...
업데이트 후 버전이 **2.11.0**인지 확인하고 새 대화 또는 새 작업을 시작하세요.
