...
- 레퍼런스 찾기, 기획, 프로젝트 설정에는 Higgsfield 크레딧이 들지 않습니다. Meigen 같은 다른 연결 도구의 요금은 각 서비스에서 확인하세요.
...
먼저 기존 설명과 첨부파일에서 목적·대상·게시 위치를 파악합니다. 부족한 핵심 정보만 1~3개 질문하고, 이미 알려 준 내용은 다시 묻지 않습니다. "구매하게", "예약 문의가 오게", "이해하고 저장하게"처럼 말하면 됩니다. 배경 제거나 부분 편집은 마케팅 인터뷰 없이 필요한 수정만 확인합니다.

1. **레퍼런스 활용**: 원하는 참고 사진이 있으면 먼저 분석하고 새 검색은 생략합니다. 새 연출 사진에 참고 자료가 필요하면 제품에 어울리는 레퍼런스(Pinterest 5장 + Meigen 5장)를 찾아 보여주고 한 장을 추천합니다. 다른 사진이 마음에 들면 번호만 말하면 됩니다. 광고 문구는 알려 주신 사실로만 쓰고, 말하지 않은 가격이나 할인은 넣지 않습니다.
2. **확인 카드 1장**: 만들 것, 레퍼런스, 방향, 들어갈 문구, 사용 도구, 1장·2장 비용을 한 번에 보여줍니다. "이대로 1장 만들기" 또는 "2장 만들어 더 나은 것 고르기"를 골라야 생성이 시작됩니다. 필요하면 제품 배경을 먼저 지워 깔끔하게 합성합니다.
   - **참고 사진 보내기**: 레퍼런스와 더 닮게 만들려고 고른 레퍼런스 사진을 Higgsfield에 참고 입력으로 보낼 때는 카드에 "다른 사람의 작품을 보낸다"는 줄이 따로 나옵니다. 그 카드를 승인해야만 보내고, 원하지 않으면 "사진은 보내지 마"라고 하면 됩니다. 레퍼런스는 대부분 다른 사람의 작품이라 결과가 원본과 너무 비슷하면 광고에 쓰지 마세요.
3. **결과와 제품 확인**: 완성본과 함께 모양·색상·라벨 글자·로고·개수가 원본 제품과 같은지 ✓로 보여줍니다. 이어서 다른 비율(1:1·4:5·9:16)로 늘리기, 이 이미지로 영상 만들기를 제안하고, 결과가 쓰려는 곳보다 작을 때만 고화질(2K·4K)로 키우기를 권합니다. 비율 늘리기와 고화질 키우기는 Higgsfield가 제공하는 기능을 쓰며, 이 플러그인에서는 아직 실제 시험 전입니다. 모두 따로 비용을 확인한 뒤 진행합니다.

새 기준 이미지와 영상을 모두 생성할 때는 확인 카드가 두 번 나옵니다. 이미 승인된 이미지는 재사용하고, 기존 영상의 부분 수정에 불필요한 이미지 생성을 추가하지 않습니다. 여러 방향을 비교해 보고 싶다면 "여러 안 보여줘", "콘셉트 3안"이라고 말하세요. 자세히 보기 방식으로 바뀝니다. 레퍼런스를 직접 고르고 싶으면 "레퍼런스 먼저 보여줘", 레퍼런스 없이 바로 만들고 싶으면 "레퍼런스 없이 바로 만들어줘"라고 하면 됩니다. 레퍼런스는 사이트를 정하지 않으면 Pinterest 5장 + Meigen 5장, 총 10장을 보여줍니다.

**전문 용어를 공부할 필요가 없습니다**: "배경만 흐리게", "이 참고 사진처럼 내 제품으로"라고 말하면 연출과 프롬프트는 내부에서 정리합니다. 필요한 것만 쉬운 선택지로 물어보고 "알아서 추천해줘"도 가능합니다. 제공한 참고 사진이 있으면 새 검색을 강요하지 않습니다. 용어와 전문 프롬프트는 요청할 때만 설명합니다.

캐러셀·썸네일·광고는 목적별 제작 지침을 사용합니다. 사이트 트렌드 검색은 실행하지 않으며, 첨부 레퍼런스와 제품 원본에 맞는 구성·가독성·일관성을 검수합니다. 일반 레퍼런스 기본값은 Pinterest 5 + MeiGen 5로 유지합니다.

**모델도 몰라도 됩니다**: 새로 생성할지, 부분만 수정할지, 일반 편집으로 처리할지를 먼저 판단합니다. 이미지 기본값은 GPT Image 2.5이며 다른 모델이 더 적합한 구체적 이유가 있으면 이유·적용 범위·비용을 같은 확인 카드에서 제안합니다. 승인 없이 바꾸거나 여러 모델을 연달아 호출하지 않습니다. [목적별 요청과 편집 예시](docs/USER-GUIDE.md#목적을-말하면-제작-방식까지-추천해요).

**광고 템플릿**: "Higgsfield 광고 템플릿으로 만들어줘"라고 하면 Higgsfield 광고 사진 템플릿 중 제품에 어울리는 것을 골라 비슷한 분위기로 만들 수 있습니다(비율은 템플릿이 지원하는 것만 가능). 대화 안 생성은 Claude 작업 환경의 Higgsfield 연결에서 확인했고, Claude Cowork·ChatGPT Work는 연결에 이 기능이 보이는지 확인이 필요합니다. 보이지 않으면 Higgsfield 웹에서 마무리하도록 안내합니다.
...
업종별 구매 여정·증거·연출·컴플라이언스 판단과 공통 제작 워크플로를 결합해 **광고·홍보·마케팅·상세/랜딩·콘텐츠**를 기획하고 제작하는 ChatGPT Work/Codex 및 Claude 플러그인입니다. Chany's Studio 2.15.0은 목적을 쉬운 말로 파악해 생성·편집 방식과 모델을 추천하고, 첨부 레퍼런스·잡지형 캐러셀·썸네일·영상 부분 편집을 연결합니다. GPT Image 2.5 기본값, 제품 바이럴 리메이크·광고 변형·숏폼 재가공·제품 3D와 유료 실행의 현재 연결·입력·비용 확인을 유지합니다.

**처음 쓰는 분:** [왕초보 실습 교안](https://chany-studio.chanjunpark.chatgpt.site/learn.html)에서 플러그인 사용법과 기본 개념부터 첫 이미지·영상, 수정·저장까지 따라 하세요. [교안 원문](docs/BEGINNER-WORKBOOK.md)에는 14종 범용 상세 요청문과 90분 수업안도 있습니다.
...
| ChatGPT `@chany-ad-creative` · Codex `$chany-ad-creative` | 단일 이미지·광고/잡지형 캐러셀·유튜브 썸네일·인스타 커버·포스터·배너 |
...
Claude의 대화형 환경에서는 호출 내용·첨부파일·기존 프로젝트 기록을 먼저 확인한 뒤, 빠진 캠페인 정보만 네이티브 **Ask your question** 카드에 한 번에 최대 3개씩 표시합니다. 설계안이 완성되면 `승인 후 생성 (권장)`·`설계 수정`·`취소` 카드에서 선택해야 Chany 관리 파일 쓰기가 시작됩니다. 질문 카드 기능이 현재 호스트에 없거나 답이 비어 있으면 이를 승인으로 간주하지 않고 같은 최소 질문을 일반 대화로 이어가며, 명시적으로 취소하면 재질문 없이 끝납니다.
...
사이트 트렌드 검색은 제작 흐름에서 제외합니다. 기존 레퍼런스 검색과 첨부 사진을 바탕으로 브랜드·메시지·가독성에 맞게 연출하고, 기록된 스타일 예시는 최신 유행이라고 표시하지 않습니다. 필요한 플랫폼 규격과 도구 지원 여부만 공식 문서에서 확인합니다.
...
생성형 정적 이미지와 부분 편집의 기본 모델은 **GPT Image 2.5**, 정확한 모델 ID는 `gpt_image_2_5`입니다. 호출 전 현재 연결에서 실제로 선택 가능한지 확인하며, 확인되지 않은 공급자 기본값을 GPT Image 2.5라고 부르지 않습니다. 사용자 지정·승인된 프로젝트 정책·필수 기능 부재 외에도 작업에 더 적합한 구체적 이유가 있으면 대체 모델을 제안할 수 있습니다. 실제 실행은 해당 산출물·입력·비용을 담은 확인 카드를 승인한 뒤에만 합니다. 근거가 불확실하면 기본값을 유지합니다. 영상·음성·일반 편집·클립 조립에는 이 정적 이미지 기본값을 강제하지 않습니다.
...
업데이트 후 버전이 **2.15.0**인지 확인하고 새 대화 또는 새 작업을 시작하세요.
