...
생성형 정적 이미지와 부분 편집의 기본 모델은 **GPT Image 2**, 정확한 모델 ID는 `gpt-image-2`입니다. 호출 전 현재 도구 스키마에서 이 모델을 실제로 선택할 수 있는지 확인하고, 요청 기본값과 실제 해석된 모델을 함께 표시합니다. 모델 선택기가 숨겨져 있고 현재 문서나 런타임 정보로 GPT Image 2임을 확인할 수 없으면 공급자 기본값을 GPT Image 2라고 부르지 않습니다. 사용자가 다른 모델을 지정했거나, 승인된 프로젝트 정책이 있거나, 필수 작업을 `gpt-image-2`가 지원하지 않는다고 라이브 확인된 경우에만 해당 산출물 범위에서 대체 모델을 승인받아 사용합니다. 영상·음성·클립 조립에는 이 정적 이미지 기본값을 강제하지 않습니다.

유료 생성 전에는 연결된 도구의 **현재 스키마와 라이브 카탈로그**에서 실제 작업, 모델 또는 워크플로, 입력 역할, 옵션과 제한을 확인합니다. 무료 견적·비용 미리보기가 있으면 먼저 실행하고, 최종 프롬프트, 입력별 역할, 확인된 모델·워크플로와 옵션, 서버 조정값, 공급자가 반환한 크레딧·잔액 정보, 출력 수와 배치 한도를 한 번에 보여준 뒤 승인을 받습니다. `gpt-image-2` 기본 정책을 제외한 기억 속 모델명이나 이전 가격표를 런타임 사실처럼 사용하지 않습니다. 모델이나 공급자를 바꾸면 기존 견적·유료 생성 승인·크리에이티브 승인은 해당 범위에서 무효가 되므로 다시 확인합니다.
...
- Higgsfield MCP가 연결된 Claude에서는 영상·Soul 캐릭터·오디오와 UGC 워크플로를 실행할 수 있습니다. 정적 이미지에 Higgsfield나 다른 공급자를 쓰려면 위 조건을 충족한 범위별 대체 승인이 필요합니다.
